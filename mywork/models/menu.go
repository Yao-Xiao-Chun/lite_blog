...
type LiteAdminMenu struct {
	...
	Menu_roule int `gorm:"default:0;not null"` //菜单类型 0代表顶级 1代表一级分类

	Menu_parent int `gorm:"null"` // 所属父类 默认为空

	Menu_status int `gorm:"not null;default:1"` // 启用状态 1代表启用 0代表禁用

	Menu_key string `gorm:"not null;default:'-'"` //菜单key

	Menu_level int `gorm:"not null;default:0"` //菜单等级 |-
