 ...
 */
 func AddArticle(article LiteArticle)(articles LiteArticle,err error){
 	...
 	return article,db.Create(&article).Error
  ...
  */
  func EditArticle(article LiteArticle) bool{
  	...
  	db.Omit("created_at","click","read_num").Save(&article) //更新的时候忽略某个字段
  ...
  */
  func GetCountArticle() (count int,err error){
  	...
  	return count,db.Where("status = ?",1).Find(&article).Count(&count).Error
  }
   ...
   */
   func SelectArticle(page string)(articles []LiteArticle,err error){
	   ...
	   return articles,db.Select("created_at,id,title,priority,is_top,status,click,read_num,author,fid_level").Order("created_at desc,id desc").Offset((pages-1)*limit).Limit(limit).Find(&articles).Error
   }
    ...
    */
    func FindArticleInfo(id int)(article LiteArticle,err error){
    	...
    	return article,db.Where("id = ?",id).Limit(1).Find(&article).Error
	}
	 ...
	 */
	 func GetHomeArticleInfo(id int)(article LiteArticle,err error){
		 ...
		 return article,db.Where("status = ? and id = ?",1,id).Limit(1).Select("created_at,title,priority,click,read_num,keywords,author,id,content").Find(&article).Error
	  ...
	  */
	   ...
	   */
	   func SetArticleAndRead(id int){
	   		...
	   		db.Model(&article).Where("id = ? and status = ?",id,1).Update("read_num",gorm.Expr("read_num + 1"))
